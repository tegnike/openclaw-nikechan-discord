...
### AGENTS.md — 運用ルール（変更不可）
**書く:** セッション開始手順、メモリシステムの使い方、安全規則、グループチャットの振る舞い、ハートビート運用
**書かない:** 性格や価値観（→SOUL.md）、ツール詳細（→TOOLS.md）、定期タスクの具体項目（→HEARTBEAT.md）
**注意:** OpenClawのテンプレートファイル。エージェントが自分で変更してはいけない。カスタマイズはSOUL.md等の他ファイルで行う
...
- **Heartbeat向き:** 数時間おきのチェック、複数タスクのバッチ実行、会話コンテキストが必要なもの（例: ブログ監視）
- **Cron向き:** 1日1回など低頻度の定期タスク、正確なタイミングが必要なもの、重い処理（例: 曲作成、宣伝）
- Heartbeatは~30分間隔で発火するため、1日1回のタスクを書くと残り47回は「やることなし」の無駄なチェックになる
- 1日1回のタスクは `cron/jobs.json` に登録すること
